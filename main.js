...
templates.movie = [
  ...
  "<img src='<%= movie.img %>'/>",
  "<p> <%= movie.desc %> </p>",
  "<b><h3>Directors: </h3></b><p> <%= movie.director %> </p>",
  "<b><h3>Stars: </h3></b><p> <%= movie.stars %> </p>",
...
templates.editMovie = [
  '<form class="formEdit" data-id="<%= id %>">',
    ...
    '<input type="submit" class="submitEdit" name="name" value="Update">',
  '</form>'
].join('');

templates.addMovie = [
  '<form class="formAdd" data-id="<%= id %>">',
    '<h2>Create your own movie submission here</h2>',
    'Title:<input class="titleAdd" type="text" value="<%= title %>">',
    'Director:<input class="directorAdd" type="text" value="<%= director %>">',
    'Stars:<input class="starsAdd" type="text" value="<%= stars %>">',
    'Add Image:<input class="imageAdd" type="text" value="<%= img %>">',
    '<textarea class="descAdd" name="esc" placeholder="<%= desc %>"></textarea>',
    '<input type="submit" class="submitAdd" name="name" value="Update">',
  '</form>'
].join('');
...
$(document).ready(function() {
  ...
  //
  // var formMarkup = new FormView();
  // $('target').html(formMarkup.render().el);
});
...
var page = {
  ...
  initEvents: function() {
    ...
    editMovie: function() {
      ...
      $(this).parent('article').replaceWith(editMovietmpl(myModel.toJSON()));
    },
